...
public class SLinkedList<E extends DeepClone<E>> implements Iterable<E>
{
	...
	/*
 	...
 	{
 		/**** ADD CODE HERE ****/
 		/*Created an empty list to store the deep copy and an iterator object to loop through the list
 		 * 
 		 * We loop while there is a next element to copy in the list
 		 * If true then we create a node catsed to type E and we use deepClone to make a deepCopy of it
 		 * We use addLast to add at the end of our list. This way we save time complexity and we don't worry about indexes
 		 * 
 		 * We return the deepCopy
 		 */
 		SLinkedList<E> dCopy = new SLinkedList<E>();
 		Iterator itr = this.iterator();
 		
 		while(itr.hasNext()) {
 			E node=((E) itr.next()).deepClone();
 			dCopy.addLast(node);
 		}
