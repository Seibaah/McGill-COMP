...
int main(int argc, char *argv[])
{
	...
	//TA Mushfique said we could put reorder test 2 and 3
	...
	//modified to c2[8] since I allocate at the bottom
	if (cp3 == c2[8] && cp3 != NULL)
	{
		if (cp4 == c2[19])
		{
			// s printf(str, "C[19]: %p", c[19]);
			// puts(str);
	...
	if(cp3 != NULL && cp4 != NULL) {
		...
		//move the pointer to allow write 
		int *dummy=cp3+1;
		int *dummy2=cp4+1;
